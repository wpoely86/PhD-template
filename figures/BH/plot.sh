...
for file in orbs-scan-*.txt ; do
    nn=$(echo $file | sed "s:^.*\([0-9]-[0-9]\).*$:\1:")
    outname="$(basename $file .txt)"
    echo $nn
    cat > plot-$outname.gnu << EOF
...
#    set title "Between orbitals ${nn}"
    ...
    plot '${file}' u 1:2 w l t 'Jacobi rotation' ls 1, '' u 1:3 w l t 'v2DM + Jacobi rotation' ls 2
EOF

    gnuplot "plot-$outname.gnu"
